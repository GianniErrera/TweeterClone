<form method="POST" action= {{ route('like.tweet',
    ...
    <div class="flex items-center mt-1 mr-4">
        ...
        <span class="text-xs {{ $tweet->isLikedBy(current_user()) ? "text-blue-500" : "text-gray-500" }}">
            {{ $tweet->likes ?: 0}}
        </span>
    </div>
</form>
<form method="POST" action= {{ route('dislike.tweet',
    ...
    <div class="flex items-center mt-1">
        ...
        <span class="text-xs {{ $tweet->isDislikedBy(current_user()) ? "text-blue-500" : "text-gray-500" }}">
            {{ $tweet->dislikes ?: 0}}
        </span>
    </div>
</form>
